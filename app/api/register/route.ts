...
export async function POST(request: NextRequest) {
  ...
  if (isDuplicate) {
    return Response.json({
      success: true,
      message: "Ya tienes una cuenta Sandbox con este email.",
    });
  }
  ...
  await resend.emails.send({
    ...
    to: email,
    subject: "Cuenta Sandbox creada — @jaijtech/verifactu",
    html: `
      <div style="font-family: system-ui, -apple-system, sans-serif; max-width: 520px; margin: 0 auto; color: #1a1a1a;">
        ...
        <p>Hemos creado tu cuenta Sandbox en <strong>@jaijtech/verifactu</strong>.</p>
  ...
  await resend.emails.send({
    ...
    subject: `Nuevo registro Sandbox — ${name} (${company || "sin empresa"})`,
    html: `
      <div style="font-family: system-ui, -apple-system, sans-serif; color: #1a1a1a;">
        <h2 style="margin-top: 0;">Nuevo registro Sandbox</h2>
